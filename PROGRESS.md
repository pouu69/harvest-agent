...
> **Task 1–17, 19, 21, 22b 완료 (20/25). 다음 세션 재개 지점: Task 18 (MCP server wrap)** — [§ 다음 세션 재개 가이드](#다음-세션-재개-가이드) 참고.
...
테스트: 391/391 pass. `npm run typecheck && npm test && npm run lint && npm run build` 전부 통과.
...
### Phase 2 — 도구 (in-process MCP) — **18 만 남음**
...
| 18 | **MCP server wrap** | ⏳ pending | `src/tools/server.ts` — `createSdkMcpServer({ name: "harvest", tools: [13개] })`, `HARVEST_TOOL_NAMES` const, `tool(name, desc, schema.shape, handler)` 시그니처 사용 (SPEC_DEFECTS O-3 참고). 의존성: Tasks 14–17 (모두 완료), 22b (LLM caller; 완료). 참조: §10.1. |
...
### 현재 상태 스냅샷 (commit `14e8e12` 기준)

- **테스트**: 391/391 pass. typecheck/lint/build 전부 green.
- **완료**: 20/25 task (1–17, 19, 21, 22b).
- **남은 5개**: Task 18 (MCP server wrap) → Task 20 (`harvest start`) → Task 22a/22c/22d (안정성/문서/배포).
- **의존 그래프**: 18 → 20 → 22a → 22c/22d. 18 가 시작점, 22c/22d 는 마지막.

### 권장 재개 순서

1. **Task 18 (MCP server wrap)** — 1개 파일. `createSdkMcpServer({ name: "harvest", tools: [13개] })`. SDK 의 `tool(name, desc, schema.shape, handler)` 시그니처는 raw shape 받음 (SPEC_DEFECTS **O-3**); 13개 도구의 export 된 `z.object({...})` schema 에서 `.shape` 추출 후 wrap. `HARVEST_TOOL_NAMES = ["mcp__harvest__list_unprocessed_sessions", ...]` 상수 (§10.1 line 1783). 의존성 모두 완료.
2. **Task 20 (`harvest start`)** — 가장 큰 통합 task. CLI argv → `--recent N` (P-1) + `--discover` (I-5) + `--dry-run` 추가. Agent SDK `query()` 호출 (mode by env, replay/mock for tests). report_progress 핸들러가 stdout 출력하므로 controller 추가 처리 X. SIGINT → `releaseLocks` + INDEX 재빌드. `cwd_filter` 도입 (I-4 해소).
3. **Task 22a (시나리오 픽스처)** — `tests/fixtures/scenarios/01-single-kb-single-session/` 만들기. transcripts/, kb-initial/, expected-properties.yaml. 50%-fail 측정 → I-8 retry 결정 트리거.
4. **Task 22c (README)** — install/init/start + 보안 note (§15.3 transcript 가 LLM 으로 전송됨). DESIGN_PROPOSALS.md 의 P-3 (paths 제거 검토) 도 README 의 "v2 후보" 섹션으로 옮기기.
5. **Task 22d (npm publish)** — `package.json` polish, `.npmignore`, bin shebang 검증.
