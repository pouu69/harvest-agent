...
| 20 | `harvest start` end-to-end | `46335d2` + fix `fdca359` | `src/cli/start.ts` (KB chain resolve, `--discover` walk, `--dry-run` short-circuit, SIGINT cleanup `cleanupOnSignal` — `unlinkSync` lock + `writeFileSync` INDEX rebuild) + `src/agent/runner.ts` (per-KB sequential lock, lazy `query()` import, `cwd_filter` (I-4) + `discover_path` (I-5) kickoff prompt) + `src/agent/message-handler.ts` (§10.3 SDKMessage dispatcher) + `src/cli/argv.ts` `--recent N` (P-1) wired. 49 tests. ✅ spec + ✅ quality (post fix). |
| 22a | 시나리오 픽스처 + EXTRACT 검증 | `4638a7a` + fix `029ab15` | `tests/fixtures/scenarios/01-single-kb-single-session/` (transcripts/sess-jwt-refresh-loop-001.jsonl, kb-initial/.harvest/, llm-responses/, expected-properties.yaml, README.md) + `tests/scenarios/01-single-kb-single-session.test.ts`. YAML-driven assertions, 3-run loop (1 happy + 2 distinct validator-fail). **I-8 측정 결과**: hand-authored fixture 로는 retry 효과 검증 불가 (replay 결정론적); validator-rejection 경로 도달 가능성만 입증. 실제 측정은 post-v1 record-mode. |
| 22c | README + CHANGELOG | `494b34f` + 보강 | `README.md` (install / quick start / 명령어 / 보안 / v2 후보 / 개발 / license), `CHANGELOG.md` ([0.1.0] entry). 기본 작성은 stale base 에서 cherry-pick 후 main 기준으로 보강 (Task 20/22a/22d 반영). |
| 22d | npm publish setup | `9bd2558` | `package.json` (name `harvest-cli`, version `0.1.0`, `bin`/`files`/`engines: node >=20`/`keywords`/`license: MIT`/`prepublishOnly` gate), `LICENSE` (MIT), `.npmignore` (src/ tests/ spec 문서 제외), `src/cli/index.ts` `--version 0.1.0` 동기화, tsup banner shebang 검증. |
...
| 20 | `harvest start` end-to-end | ✅ 완료 | `src/cli/start.ts` + `src/agent/runner.ts` + `src/agent/message-handler.ts`. `cwd_filter` (I-4) + `discover_path` (I-5) 해소. SIGINT cleanup (lock unlink + INDEX writeFileSync). |
| 21 | CLAUDE.md marker integration 강화 | ✅ 완료 | `src/claudemd/integration.ts` |

### Phase 4 — 안정성 / 품질 / 배포 — ✅ 완료

| # | Task | 상태 | 핵심 산출물 |
|---|---|---|---|
| 22a | 단위 테스트 + 시나리오 픽스처 | ✅ 완료 | `tests/fixtures/scenarios/01-single-kb-single-session/` + `tests/scenarios/01-single-kb-single-session.test.ts`. I-8 retry 결정은 post-v1 record-mode 측정으로 보류. |
| 22b | Recording/Replay LLM 모드 | ✅ 완료 | `src/llm/` |
| 22c | README + docs | ✅ 완료 | `README.md`, `CHANGELOG.md` |
| 22d | npm publish setup | ✅ 완료 | `package.json` (`0.1.0`), `LICENSE` (MIT), `.npmignore` |

---

## 다음 세션 재개 가이드

### 현재 상태 스냅샷 (commit 최신; 25/25 완료)

- **테스트**: 449/449 pass. typecheck/lint/build 전부 green.
- **완료**: 25/25 task. v0.1.0 ready.
- **남은 publish 단계** (코드 작업 X): `package.json` 의 `repository` URL `<owner>/<repo>` placeholder 를 실 GitHub repo 로 교체 → `npm publish` (이미 `prepublishOnly` 가 모든 게이트 자동 검증).

### Post-v1 후보 (별도 추적)

- **P-3 (DESIGN_PROPOSALS)**: frontmatter `paths` 제거 검토 — 첫 release 후 stale 빈도 측정 후 결정.
- **I-8 (SPEC_DEFECTS)**: EXTRACT 50%-fail retry — record 모드 정기 통합 테스트에서 실 LLM noise 측정 후 결정.
- **I-9 (SPEC_DEFECTS)**: `cross_kb_id_format_error` 의 promote-item I/O catch-all 오용 → `origin_update_failed` / `origin_unlink_failed` 신규 코드 도입.
- **`harvest start --dry-run` 의 완전한 단락 보장** — 현재는 의도 보고만, 일부 쓰기를 완전히 단락하지 않을 수 있음.
