...
> **Task 1–22b 완료 (23/25). 남은 2개: Task 22c (README), 22d (npm publish)** — [§ 다음 세션 재개 가이드](#다음-세션-재개-가이드) 참고.
...
| 22a | 시나리오 픽스처 + EXTRACT 검증 | `4638a7a` | `tests/fixtures/scenarios/01-single-kb-single-session/` (transcripts/sess-jwt-refresh-loop-001.jsonl, kb-initial/.harvest/, llm-responses/, expected-properties.yaml, README.md) + `tests/scenarios/01-single-kb-single-session.test.ts`. **I-8 측정 결과**: hand-authored fixture 로는 retry 효과 검증 불가 (replay 결정론적); validator-rejection 경로 도달 가능성만 입증. 실제 측정은 post-v1 record-mode. |
...
### 현재 상태 스냅샷 (commit `fdca359` 기준; Task 22a fix 진행 중)

- **테스트**: 449/449 pass. typecheck/lint/build 전부 green.
- **완료**: 23/25 task (1–22b).
- **남은 2개**: Task 22c (README) + Task 22d (npm publish). 둘 다 병렬 실행 가능.

### 권장 재개 순서

1. **Task 22c (README)** — install/init/start + 보안 note (§15.3 transcript 가 LLM 으로 전송됨). DESIGN_PROPOSALS.md 의 P-3 (paths 제거 검토) 도 README 의 "v2 후보" 섹션으로 옮기기.
2. **Task 22d (npm publish)** — `package.json` polish, `.npmignore`, bin shebang 검증.
