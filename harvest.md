...
| 실패 종류 | 처리 |
|---|---|
| Agent SDK 자체 에러 (네트워크, auth) | 즉시 종료 (exit 5). lock 해제. 다음 실행에서 재시도 |
...
| 환경 변수 | 기본 | 의미 |
|---|---|---|
| `HARVEST_MODEL` | `claude-sonnet-4-6` | Agent 모델 |
| `HARVEST_EXTRACT_MODEL` | (HARVEST_MODEL 따름) | extract_items_from_transcript 내부 LLM |

Opus는 Agent에 권장 안 함 — 비용 대비 효과 불분명. 단순 작업은 Haiku 가능하나 도구 호출 정확도 떨어질 수 있음. **Sonnet이 sweet spot**.
...
| `ANTHROPIC_API_KEY` 환경변수 없음 | 안내 메시지 + exit 5 |
...
| 에러 | 처리 |
|---|---|
| Agent SDK API 실패 (network) | SDK 자동 재시도 (built-in). 그래도 실패 시 도구가 `llm_call_failed` 반환 → Agent는 직접 추출 모드로 전환 |
| Agent SDK API 실패 (auth) | 즉시 throw → 도구가 throw → CLI 종료 (exit 5와 같음) |
