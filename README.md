...
> **참고**: 본 프로젝트는 pre-release. npm 등록은 Task 22d 에서 진행 예정. 지금은 dev 설치를 권장.
...
> **현재 상태**: `harvest start` 는 Task 20 에서 end-to-end wiring 진행 중. argv 파서와 in-process MCP server (13 도구) 는 이미 구현되어 있다.
...
본 프로젝트는 **pre-release** 다. 25개 구현 task 중 **22개 완료**, 남은 항목:

- **Task 20** — `harvest start` end-to-end wiring (Agent SDK `query()` 호출, message stream 처리, SIGINT cleanup).
- **Task 22a** — 시나리오 픽스처 + EXTRACT 50%-fail 측정.
- **Task 22d** — npm publish setup (`package.json` polish, `.npmignore`, license).

현재 동작하는 것:
- `harvest init` (단일 KB / `--scan` 모노레포 / `--root` 마커) — 완전 동작.
- argv 파서, exit code, env vars — 완전 동작.
- in-process MCP server + 13 개 도구 — 완전 동작 (`harvest start` 가 wire-up 되면 즉시 가용).
- LLM caller 4 모드 (live / mock / replay / record) — 완전 동작.

400/400 단위 테스트 통과. typecheck / lint / build 모두 green.
...
- **I-8 (post-v1)**: EXTRACT 50%-fail 1회 재시도. 실 LLM 측정값 (Task 22a 시나리오 픽스처가 record 모드로 측정) 후 도입 결정. 자세한 내용은 [`SPEC_DEFECTS.md`](./SPEC_DEFECTS.md) I-8 참고.
...
TBD (Task 22d 에서 npm publish 전 확정 예정 — MIT 후보).
